...
type State = {
  filterCount: number;
  postModel: PostModel<ExpenseSelectPageRequest>;
};

type Action = {
  handleFiltering: (filterModel: ExpenseSelectPageRequest) => void;
  handlePagination: (pageModel: PaginationModel) => void;
  handleSorting: (sortModel: SortModel) => void;
};

export const useExpenseStore = create<State & Action>()(
  persist(
    set => ({
      postModel: {
        pageOffset: 0,
      ...
      filterCount: 0,
      handleFiltering: (filterModel: ExpenseSelectPageRequest) =>
        set(state => ({
